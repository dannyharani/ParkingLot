	...
	public void simulate() {
		...
		while (clock < steps) {
			...
			if(RandomGenerator.eventOccurred(probabilityOfArrivalPerSec)) {
				this.incomingQueue.enqueue(new Spot(RandomGenerator.generateRandomCar(), clock));
			}
			...
			for(int i = 0; i < lot.getNumRows(); i++) {
				for(int j = 0; j < lot.getNumSpotsPerRow(); j++) {
					...
					if(lot.getSpotAt(i, j) != null ) {
						TimeSinceParked = clock - lot.getSpotAt(i, j).getTimestamp();	
						if( TimeSinceParked == MAX_PARKING_DURATION) {
							outgoingQueue.enqueue(lot.remove(i, j));
						} else {
							if(RandomGenerator.eventOccurred(departurePDF.pdf(TimeSinceParked))){
								outgoingQueue.enqueue(lot.remove(i, j));
							}
						}
					}

					if(car != null || !incomingQueue.isEmpty()) {
						if(car == null) {
							car = incomingQueue.dequeue().getCar();
						}

						if(lot.attemptParking(car, clock)) {
							System.out.println(car + " ENTERED at timestep " + clock + "; occupancy is at " + lot.getTotalOccupancy());
							car = null;
						}
					...
					if(!outgoingQueue.isEmpty()) {
						OutCar = outgoingQueue.dequeue().getCar();
						System.out.println(OutCar + " EXITED at timestep " + clock + "; occupancy is at " + lot.getTotalOccupancy());
					}
